...
namespace AcademiaDoZe.Infrastructure.Tests
{
    public class LogradouroInfrastructureTests : TestBase
    {
        ...
        [Fact] public async Task AtualizarAlteraRegistro() { var item = await Inserir(); var updated = Logradouro.Criar(item.Id, GerarCep(), "Rua Atualizada", "Bairro Novo", NomeSgbdAtual, "SC", "Brasil").Value!; await repository.Atualizar(updated); var found = await repository.ObterPorId(item.Id); Assert.Equal("Rua Atualizada", found!.Nome); }
        [Fact] public async Task AtualizarInexistenteLancaExcecao() { var item = Logradouro.Criar(999999, GerarCep(), "Rua", "Bairro", NomeSgbdAtual, "SC", "Brasil").Value!; var ex = await Assert.ThrowsAsync<InfrastructureException>(() => repository.Atualizar(item)); Assert.Equal("REGISTRO_NAO_ENCONTRADO", ex.ErrorCode); }
        ...
        [Fact] public async Task ObterPorCidadeFiltra() { var city = NomeSgbdAtual + "_" + Guid.NewGuid().ToString("N")[..6]; await Inserir(cidade: city); Assert.Single(await repository.ObterPorCidade(city)); Assert.Empty(await repository.ObterPorCidade("Cidade inexistente")); }
        [Fact] public async Task ObterPorBairroFiltra() { var city = NomeSgbdAtual + "_" + Guid.NewGuid().ToString("N")[..6]; var district = "Cidade Xavier_" + Guid.NewGuid().ToString("N")[..6]; await Inserir(bairro: district, cidade: city); Assert.Single(await repository.ObterPorBairro(city, district)); Assert.Empty(await repository.ObterPorBairro(city, "Bairro inexistente")); }
    }
}